...
// UTXOSetCheckpoint is used to verify the UTXO set consistency in a given
// height, enabling faster initial block download until the checkpoint, because
// we can skip several validations.
type UTXOSetCheckpoint struct {
	Height      int32
...
type Params struct {
	...
	// UTXOCheckpoint contains data that allows us to skip several
	// validation steps by asserting that the UTXO set hash matches the
	// hardcoded hash when the UTXO set checkpointed block is reached.
	//
	// This optimization turns possible faster initial block download,
	// specially in computers constrained in memory and using slow disks.
	//
	// This is similar to AssumeUTXO, the UTXO set hash is the same as the
	// one generated by Bitcoin Core dumptxoutset, the difference is that
	// the only external data that we need to perform IBD with UTXO set
	// checkpoint are the blocks.
	UTXOCheckpoint UTXOSetCheckpoint
