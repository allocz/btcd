...
// UTXOSetStats represents the status of the current UTXOSet.
...
// CalcUTXOSetStats flushes the utxo cache and iterate over all UTXOs, computing
// the UTXO stats. When we have the same UTXO's in the database, UTXOSetHash is
// the same as the calculated by Bitcoin Core assumeutxo snapshots.
func (b *BlockChain) CalcUTXOSetStats() (UTXOSetStats, error) {
	var z UTXOSetStats
